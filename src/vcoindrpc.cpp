...
void VcoindRPC::setConnection(Connection* c) {
    if (conn) {
        delete conn;
    }
...
bool VcoindRPC::haveConnection() {
    return conn != nullptr;
}

void VcoindRPC::fetchTAddresses(const std::function<void(json)>& cb) {
    if (conn == nullptr)
        return;
...
void VcoindRPC::fetchZAddresses(const std::function<void(json)>& cb) {
    if (conn == nullptr)
        return;
...
void VcoindRPC::fetchTransparentUnspent(const std::function<void(json)>& cb) {
    if (conn == nullptr)
        return;
...
void VcoindRPC::fetchZUnspent(const std::function<void(json)>& cb) {
    if (conn == nullptr)
        return;
...
void VcoindRPC::createNewZaddr(bool sapling, const std::function<void(json)>& cb) {
    if (conn == nullptr)
        return;
    ...
    json payload = {
        ...
        {"params", { sapling ? "sprout" : "sapling" }},
    };
    
    conn->doRPCWithDefaultErrorHandling(payload, cb);
}

void VcoindRPC::createNewTaddr(const std::function<void(json)>& cb) {
    if (conn == nullptr)
        return;
...
void VcoindRPC::fetchZPrivKey(QString addr, const std::function<void(json)>& cb) {
    if (conn == nullptr)
        return;
...
void VcoindRPC::fetchTPrivKey(QString addr, const std::function<void(json)>& cb) {
    if (conn == nullptr)
        return;
...
void VcoindRPC::importZPrivKey(QString addr, bool rescan, const std::function<void(json)>& cb) {
    if (conn == nullptr)
        return;
...
void VcoindRPC::importTPrivKey(QString addr, bool rescan, const std::function<void(json)>& cb) {
    if (conn == nullptr)
        return;
...
void VcoindRPC::validateAddress(QString address, const std::function<void(json)>& cb) {
    if (conn == nullptr)
        return;
...
void VcoindRPC::fetchBalance(const std::function<void(json)>& cb) {
    if (conn == nullptr)
        return;
...
void VcoindRPC::fetchTransactions(const std::function<void(json)>& cb) {
    if (conn == nullptr)
        return;
...
void VcoindRPC::sendZTransaction(json params, const std::function<void(json)>& cb, 
...
void VcoindRPC::fetchInfo(const std::function<void(json)>& cb, 
...
void VcoindRPC::fetchBlockchainInfo(const std::function<void(json)>& cb) {
    if (conn == nullptr)
        return;
...
void VcoindRPC::fetchNetSolOps(const std::function<void(qint64)> cb) {
    if (conn == nullptr)
        return;
...
void VcoindRPC::fetchMigrationStatus(const std::function<void(json)>& cb) {
    if (conn == nullptr)
        return;
...
void VcoindRPC::setMigrationStatus(bool enabled) {
    if (conn == nullptr)
        return;
 ...
 * private keys
 */ 
void VcoindRPC::fetchAllPrivKeys(const std::function<void(QList<QPair<QString, QString>>)> cb) {
    if (conn == nullptr) {
...
void VcoindRPC::fetchOpStatus(const std::function<void(json)>& cb) {
    if (conn == nullptr)
        return;
...
void VcoindRPC::fetchReceivedTTrans(QList<QString> txids, QList<TransactionItem> sentZTxs,
...
void VcoindRPC::fetchReceivedZTrans(QList<QString> zaddrs, const std::function<void(QString)> usedAddrFn,
        const std::function<void(QList<TransactionItem>)> txdataFn) {
    if (conn == nullptr)
        return;
