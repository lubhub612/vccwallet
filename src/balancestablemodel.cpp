...
QVariant BalancesTableModel::data(const QModelIndex &index, int role) const
{
    ...
    if (role == Qt::DisplayRole) {
        ...
        case 1: return Settings::getVCCDisplayFormat(std::get<1>(modeldata->at(index.row())));
        }
    }
    ...
    if(role == Qt::ToolTipRole) {
        switch (index.column()) {
        case 0: return AddressBook::addLabelToAddress(std::get<0>(modeldata->at(index.row())));
        case 1: return Settings::getUSDFromVccAmount(std::get<1>(modeldata->at(index.row())));
        }
    }
