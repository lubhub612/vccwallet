    ...
    ui(new Ui::MainWindow)
{
    ...
    ui->setupUi(this);
    logger = new Logger(this, QDir(QStandardPaths::writableLocation(QStandardPaths::AppDataLocation)).filePath("vcc-qt-wallet.log"));
    ...
    // Request vcoin
    QObject::connect(ui->actionRequest_vcoin, &QAction::triggered, [=]() {
        RequestDialog::showRequestVcoin(this);
    });
    ...
    // The vcoind tab is hidden by default, and only later added in if the embedded vcoind is started
    vcoindtab = ui->tabWidget->widget(4);
...
void MainWindow::setupTurnstileDialog() {        
    ...
    QObject::connect(ui->actionTurnstile_Migration, &QAction::triggered, [=] () {
        // If the underlying vcoind has support for the migration and there is no existing migration
        // in progress, use that.         
        if (rpc->getMigrationStatus()->available) {
            Turnstile::showVcoindMigration(this);
        } else {
            // Else, do nothing
        }
    });
    ...
    QObject::connect(ui->actionSettings, &QAction::triggered, [=]() {
        ...
        QObject::connect(settings.btnClearSaved, &QCheckBox::clicked, [=]() {
            if (QMessageBox::warning(this, "Clear saved history?",
                "Shielded z-Address transactions are stored locally in your wallet, outside vcoind. You may delete this saved information safely any time for your privacy.\nDo you want to delete the saved shielded transactions now?",
        ...
        QObject::connect(settings.comboBoxTheme, &QComboBox::currentTextChanged, [=] (QString theme_name) {
            ...
            QMessageBox::information(this, tr("Restart"), tr("Please restart VccWallet to have the theme apply"), QMessageBox::Ok);
        });
        ...
        if (rpc->getEVcoinD() == nullptr) {
            settings.chkTor->setEnabled(false);
            settings.lblTor->setEnabled(false);
            QString tooltip = tr("Tor configuration is available only when running an embedded vcoind.");
        ...
        // If values are coming from vcoin.conf, then disable all the fields
        auto vcoinConfLocation = Settings::getInstance()->getVcoindConfLocation();
        if (!vcoinConfLocation.isEmpty()) {
            ...
            settings.confMsg->setText("No local vcoin.conf found. Please configure connection manually.");
        ...
        // Enable the troubleshooting options only if using embedded vcoind
        if (!rpc->isEmbedded()) {
            settings.chkRescan->setEnabled(false);
            settings.chkRescan->setToolTip(tr("You're using an external vcoind. Please restart vcoind with -rescan"));

            settings.chkReindex->setEnabled(false);
            settings.chkReindex->setToolTip(tr("You're using an external vcoind. Please restart vcoind with -reindex"));
        }
        ...
        if (settingsDialog.exec() == QDialog::Accepted) {
            ...
            if (!isUsingTor && settings.chkTor->isChecked()) {
                ...
                Settings::addToVcoinConf(vcoinConfLocation, "proxy=127.0.0.1:9050");
                rpc->getConnection()->config->proxy = "proxy=127.0.0.1:9050";

                QMessageBox::information(this, tr("Enable Tor"), 
                    tr("Connection over Tor has been enabled. To use this feature, you need to restart VccWallet."), 
                    QMessageBox::Ok);
            }

            if (isUsingTor && !settings.chkTor->isChecked()) {
                // If "use tor" was previously checked and now is unchecked
                Settings::removeFromVcoinConf(vcoinConfLocation, "proxy");
                rpc->getConnection()->config->proxy.clear();

                QMessageBox::information(this, tr("Disable Tor"),
                    tr("Connection over Tor has been disabled. To fully disconnect from Tor, you need to restart VccWallet."),
                    QMessageBox::Ok);
            }
            ...
            if (settings.chkRescan->isChecked()) {
                Settings::addToVcoinConf(vcoinConfLocation, "rescan=1");
                showRestartInfo = true;
            }

            if (settings.chkReindex->isChecked()) {
                Settings::addToVcoinConf(vcoinConfLocation, "reindex=1");
                showRestartInfo = true;
            }

            if (showRestartInfo) {
                auto desc = tr("VccWallet needs to restart to rescan/reindex. VccWallet will now close, please restart VccWallet to continue");
                
                QMessageBox::information(this, tr("Restart VccWallet"), desc, QMessageBox::Ok);
...
void MainWindow::donate() {
    ...
    ui->MemoTxt1->setText(tr("Thanks for supporting VccWallet!"));

    ui->statusBar->showMessage(tr("Donate 0.01 ") % Settings::getTokenName() % tr(" to support VccWallet"));
...
void MainWindow::balancesReady() {
    ...
    if (!pendingURIPayment.isEmpty()) {
        qDebug() << "Paying vcoin URI";
        payVcoinURI(pendingURIPayment);
        pendingURIPayment = "";
    }
...
bool MainWindow::eventFilter(QObject *object, QEvent *event) {
    if (event->type() == QEvent::FileOpen) {
        ...
        if (!fileEvent->url().isEmpty())
            payVcoinURI(fileEvent->url().toString());
...
// Pay the Vcoin URI by showing a confirmation window. If the URI parameter is empty, the UI
// will prompt for one. If the myAddr is empty, then the default from address is used to send
// the transaction.
void MainWindow::payVcoinURI(QString uri, QString myAddr) {
    ...
    if (uri.isEmpty()) {
        uri = QInputDialog::getText(this, tr("Paste Vcoin URI"),
            "Vcoin URI" + QString(" ").repeated(180));
    }
    ...
    if (!paymentInfo.error.isEmpty()) {
        QMessageBox::critical(this, tr("Error paying vcoin URI"), 
                tr("URI should be of the form 'vcoin:<addr>?amt=x&memo=y") + "\n" + paymentInfo.error);
        return;
    }
...
void MainWindow::importPrivKey() {
    ...
    pui.helpLbl->setText(QString() %
                        ...
                        tr("The keys will be imported into your connected vcoind node"));  
...
void MainWindow::exportTransactions() {
    ...
    QString exportName = "vcoin-transactions-" + QDateTime::currentDateTime().toString("yyyyMMdd") + ".csv";
 ...
 * This might fail for various reasons - Remote vcoind, non-standard locations, custom params passed to vcoind, many others
*/
void MainWindow::backupWalletDat() {
    if (!rpc->getConnection())
        return;

    QDir vcoindir(rpc->getConnection()->config->vcoinDir);
    QString backupDefaultName = "vcoin-wallet-backup-" + QDateTime::currentDateTime().toString("yyyyMMdd") + ".dat";

    if (Settings::getInstance()->isTestnet()) {
        vcoindir.cd("testnet3");
        backupDefaultName = "testnet-" + backupDefaultName;
    }
    
    QFile wallet(vcoindir.filePath("wallet.dat"));
    if (!wallet.exists()) {
        QMessageBox::critical(this, tr("No wallet.dat"), tr("Couldn't find the wallet.dat on this computer") + "\n" +
            tr("You need to back it up from the machine vcoind is running on"), QMessageBox::Ok);
        return;
    }
...
void MainWindow::exportKeys(QString addr) {
    ...
    QObject::connect(pui.buttonBox->button(QDialogButtonBox::Save), &QPushButton::clicked, [=] () {
        QString fileName = QFileDialog::getSaveFileName(this, tr("Save File"),
                           allKeys ? "vcoin-all-privatekeys.txt" : "vcoin-privatekey.txt");
        QFile file(fileName);
...
void MainWindow::setupVcoindTab() {    
    ui->vcoindlogo->setBasePixmap(QPixmap(":/img/res/vcoindlogo.gif"));
}
...
void MainWindow::setupTransactionsTab() {
    ...
    QObject::connect(ui->transactionsTable, &QTableView::customContextMenuRequested, [=] (QPoint pos) {
        ...
        // Payment Request
        if (!memo.isEmpty() && memo.startsWith("vcoin:")) {
...
void MainWindow::setupReceiveTab() {
    ...
    QObject::connect(ui->listReceiveAddresses, 
        ...
        ui->rcvBal->setText(Settings::getVCCUSDDisplayFormat(rpc->getModel()->getAllBalances().value(addr)));
