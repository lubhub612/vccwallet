...
void MainWindow::setupSendTab() {
    ...
    QObject::connect(ui->minerFeeAmt, &QLineEdit::textChanged, [=](auto txt) {
        ui->lblMinerFeeUSD->setText(Settings::getUSDFromVccAmount(txt.toDouble()));
    });
...
void MainWindow::inputComboTextChanged(int index) {
    ...
    auto balFmt = Settings::getVCCDisplayFormat(bal);

    ui->sendAddressBalance->setText(balFmt);
    ui->sendAddressBalanceUSD->setText(Settings::getUSDFromVccAmount(bal));
}
...
void MainWindow::amountChanged(int item, const QString& text) {
    ...
    usd->setText(Settings::getUSDFromVccAmount(text.toDouble()));
...
Tx MainWindow::createTxFromSendPage() {
    ...
    if (Settings::getInstance()->getAutoShield() && sendChangeToSapling) {
        auto saplingAddr = std::find_if(rpc->getModel()->getAllZAddresses().begin(), rpc->getModel()->getAllZAddresses().end(), [=](auto i) -> bool { 
            // We're finding a sapling address that is not one of the To addresses, because vcoin doesn't allow duplicated addresses
...
bool MainWindow::confirmTx(Tx tx, RecurringPaymentInfo* rpi) {
    ...
    for (int i=0; i < tx.toAddrs.size(); i++) {
        ...
        // Add new Address widgets instead of the same one.
        {
            ...
            Amt->setText(Settings::getVCCDisplayFormat(toAddr.amount));
            ...
            AmtUSD->setText(Settings::getUSDFromVccAmount(toAddr.amount));
    ...
    // Add fees
    {
        ...
        minerFee->setText(Settings::getVCCDisplayFormat(tx.fee));
        ...
        minerFeeUSD->setText(Settings::getUSDFromVccAmount(tx.fee));
    ...
    QString tooltip = tr("Current balance      : ") +
        Settings::getVCCUSDDisplayFormat(rpc->getModel()->getAllBalances().value(tx.fromAddr));
    tooltip += "\n" + tr("Balance after this Tx: ") +
        Settings::getVCCUSDDisplayFormat(rpc->getModel()->getAllBalances().value(tx.fromAddr) - totalSpending);
