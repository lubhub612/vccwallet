...
struct MigrationStatus {
    bool            available;     // Whether the underlying vcoind supports migration?
    bool            enabled;
...
public:
    ...
    void setEVcoind(QProcess* p);
    const QProcess* getEVcoinD() { return evcoind; }

    void refresh(bool force = false);
    void refreshAddresses();    
    
    void checkForUpdate(bool silent = true);
    void refreshVCCPrice();
    ...
    void shutdownVcoind();
    void noConnection();
    bool isEmbedded() { return evcoind != nullptr; }
...
private:
    ...
    // Sapling turnstile migration status (for the vcoind v2.0.5 tool)
