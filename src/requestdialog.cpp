...
void RequestDialog::showPaymentConfirmation(MainWindow* main, QString paymentURI) {
    ...
    if (!payInfo.error.isEmpty()) {
        QMessageBox::critical(main, tr("Error paying vcoin URI"), 
    ...
    req.txtAmountUSD->setText(Settings::getUSDFromVccAmount(req.txtAmount->text().toDouble()));
    ...
    if (d.exec() == QDialog::Accepted) {
        main->payVcoinURI(paymentURI, req.cmbMyAddress->currentText());
    }
}

// Static method that shows the request dialog
void RequestDialog::showRequestVcoin(MainWindow* main) {
    QDialog d(main);
    Ui_RequestDialog req;
    ...
    QObject::connect(req.txtAmount, &QLineEdit::textChanged, [=] (auto text) {
        req.txtAmountUSD->setText(Settings::getUSDFromVccAmount(text.toDouble()));
    });
    req.txtAmountUSD->setText(Settings::getUSDFromVccAmount(req.txtAmount->text().toDouble()));
    ...
    if (d.exec() == QDialog::Accepted) {
        // Construct a vcoin Payment URI with the data and pay it immediately.
        QString memoURI = "vcoin:" + req.cmbMyAddress->currentText()
                    + "?amt=" + Settings::getDecimalString(req.txtAmount->text().toDouble())
                    + "&memo=" + QUrl::toPercentEncoding(req.txtMemo->toPlainText());

        QString sendURI = "vcoin:" + AddressBook::addressFromAddressLabel(req.txtFrom->text()) 
                    + "?amt=0.0001"
        ...
        // sapling address
        main->payVcoinURI(sendURI, req.cmbMyAddress->currentText());
    }
}
