...
rm -f res/*.qm
$QT_STATIC/bin/lrelease vcc-qt-wallet.pro
...
ls res/*.qm | awk -F '[_.]' '{print $4}' | while read -r language ; do 
    if [ -f $QT_STATIC/translations/qtbase_$language.qm ]; then
        $QT_STATIC/bin/lconvert -o res/vcc_$language.qm $QT_STATIC/translations/qtbase_$language.qm res/vcc_qt_wallet_$language.qm
        mv res/vcc_$language.qm res/vcc_qt_wallet_$language.qm
    fi
done
