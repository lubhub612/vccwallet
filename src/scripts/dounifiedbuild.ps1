...
Remove-Item -Force -ErrorAction Ignore ./artifacts/linux-binaries-vccwallet-v$version.tar.gz
Remove-Item -Force -ErrorAction Ignore ./artifacts/linux-deb-vccwallet-v$version.deb
Remove-Item -Force -ErrorAction Ignore ./artifacts/Windows-binaries-vccwallet-v$version.zip
Remove-Item -Force -ErrorAction Ignore ./artifacts/Windows-installer-vccwallet-v$version.msi
Remove-Item -Force -ErrorAction Ignore ./artifacts/macOS-vccwallet-v$version.dmg
...
bash src/scripts/mkmacdmg.sh --qt_path ~/Qt/5.11.1/clang_64/ --version $version --vcoin_path ~/prod/vcoin --certificate "$certificate"
if (! $?) {
    Write-Output "[Error]"
    exit 1;
}
...
scp -r src/ singleapplication/ res/ ./vcc-qt-wallet.pro ./application.qrc ./LICENSE ./README.md ${server}:/tmp/zqwbuild/ | Out-Null
...
if (! (Test-Path ./artifacts/linux-binaries-vccwallet-v$version.tar.gz) -or
    ! (Test-Path ./artifacts/linux-deb-vccwallet-v$version.deb) -or
    ! (Test-Path ./artifacts/Windows-binaries-vccwallet-v$version.zip) -or
    ! (Test-Path ./artifacts/macOS-vccwallet-v$version.dmg) -or 
    ! (Test-Path ./artifacts/Windows-installer-vccwallet-v$version.msi) ) {
        Write-Host "[Error]"
        exit 1;
    }
Write-Host "[OK]"
