...
private:
    std::shared_ptr<ConnectionConfig> autoDetectVcoinConf();
    std::shared_ptr<ConnectionConfig> loadFromSettings();

    Connection* makeConnection(std::shared_ptr<ConnectionConfig> config);

    void doAutoConnect(bool tryEvcoindStart = true);
    void doManualConnect();

    void createVcoinConf();
    QString locateVcoinConfFile();
    QString vcoinConfWritableLocation();
    QString vcoinParamsDir();

    bool verifyParams();
    void downloadParams(std::function<void(void)> cb);
    void doNextDownload(std::function<void(void)> cb);
    bool startEmbeddedVcoind();

    void refreshVcoindState(Connection* connection, std::function<void(void)> refused);
...
/**
 * Represents a connection to a vcoind. It may even start a new vcoind if needed.
