...
public:
    ...
    int main(int argc, char *argv[]) {
        ...
        parser.setApplicationDescription("Shielded desktop wallet and embedded full node for Vcoin");
        ...
        // No embedded will disable the embedded vcoind node
        QCommandLineOption noembeddedOption(QStringList() << "no-embedded", "Disable embedded vcoind");
        parser.addOption(noembeddedOption);

        // Add an option to specify the conf file
            QCommandLineOption confOption(QStringList() << "conf", "Use the vcoin.conf specified instead of looking for the default one.",
                                          "confFile");
        parser.addOption(confOption);

        // Positional argument will specify a vcoin payment URI
        parser.addPositionalArgument("vcoinURI", "An optional vcoin URI to pay");

        parser.process(a);

        // Check for a positional argument indicating a vcoin payment URI
        if (a.isSecondary()) {
        ...
        QCoreApplication::setOrganizationName("vcc-qt-wallet-org");
        QCoreApplication::setApplicationName("vcc-qt-wallet");
        ...
        translator.load(QString(":/translations/res/vcc_qt_wallet_") + locale);
        ...
        if (parser.isSet(confOption)) {
            Settings::getInstance()->setUsingVcoinConf(parser.value(confOption));
        }

        w = new MainWindow();
        w->setWindowTitle("VccWallet FullNode v" + QString(APP_VERSION));

        // If there was a payment URI on the command line, pay it
        if (parser.positionalArguments().length() > 0) {
            w->payVcoinURI(parser.positionalArguments()[0]);
        }

        // Listen for any secondary instances telling us about a vcoin payment URI
        QObject::connect(&a, &SingleApplication::receivedMessage, [=] (quint32, QByteArray msg) {
            ...
            QTimer::singleShot(1, [=]() { w->payVcoinURI(uri); });            
        });   
