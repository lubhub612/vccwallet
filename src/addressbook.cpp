    ...
    QObject::connect(ab.addNew, &QPushButton::clicked, [&] () {
        ...
        if (!Settings::isValidAddress(addr)) {
            QMessageBox::critical(parent, QObject::tr("Address Format Error"), 
                QObject::tr("%1 doesn't seem to be a valid Vcoin address.")
                    .arg(addr), 
                QMessageBox::Ok);
            return;
        } 
...
void AddressBook::readFromStorage() {
    ...
    // Special. 
    // Add the default VccWallet donation address if it isn't already present
    ...
    //     allLabels.append(QPair<QString, QString>("VccWallet donation", Settings::getDonationAddr(true)));
    // }
}
