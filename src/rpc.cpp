...
void Controller::setEVcoind(QProcess* p) {
    evcoind = p;

    if (evcoind && ui->tabWidget->widget(4) == nullptr) {
        ui->tabWidget->addTab(main->vcoindtab, "vcoind");
    }
}

// Called when a connection to vcoind is available. 
void Controller::setConnection(Connection* c) {
    ...
    // See if we need to remove the reindex/rescan flags from the vcoin.conf file
    auto vcoinConfLocation = Settings::getInstance()->getVcoindConfLocation();
    Settings::removeFromVcoinConf(vcoinConfLocation, "rescan");
    Settings::removeFromVcoinConf(vcoinConfLocation, "reindex");

    // If we're allowed to get the Vcc Price, get the prices
    if (Settings::getInstance()->getAllowFetchPrices())
        refreshVCCPrice();
...
/// This will refresh all the balance data from vcoind
...
void Controller::getInfoThenRefresh(bool force) {
    ...
    zrpc->fetchInfo([=] (const json& reply) {   
        ...
        Settings::getInstance()->setVcoindVersion(version);
        ...
        zrpc->fetchBlockchainInfo([=](const json& reply) {
            ...
            // Update vcoind tab if it exists
            if (evcoind) {
                if (isSyncing) {
            ...
            auto vccPrice = Settings::getInstance()->getUSDFromVccAmount(1);
            QString tooltip;
            if (connections > 0) {
                tooltip = QObject::tr("Connected to vcoind");
            }
            else {
                tooltip = QObject::tr("vcoind has no peer connections");
            }
            tooltip = tooltip % "(v " % QString::number(Settings::getInstance()->getVcoindVersion()) % ")";

            if (!vccPrice.isEmpty()) {
                tooltip = "1 " % Settings::getTokenName() % " = " % vccPrice % "\n" % tooltip;
            }
        ...
        // vcoind has probably disappeared.
        this->noConnection();
        ...
        if (!shown && prevCallSucceeded) { // show error only first time
            shown = true;
            QMessageBox::critical(main, QObject::tr("Connection Error"), QObject::tr("There was an error connecting to vcoind. The error was") + ": \n\n"
...
void Controller::refreshMigration() {
    // Turnstile migration is only supported in vcoind v2.0.5 and above
    if (Settings::getInstance()->getVcoindVersion() < 2000552)
        return;
...
void Controller::refreshBalances() {    
    ...
    zrpc->fetchBalance([=] (json reply) {    
        ...
        ui->balSheilded   ->setText(Settings::getVCCDisplayFormat(balZ));
        ui->balTransparent->setText(Settings::getVCCDisplayFormat(balT));
        ui->balTotal      ->setText(Settings::getVCCDisplayFormat(balTotal));


        ui->balSheilded   ->setToolTip(Settings::getVCCDisplayFormat(balZ));
        ui->balTransparent->setToolTip(Settings::getVCCDisplayFormat(balT));
        ui->balTotal      ->setToolTip(Settings::getVCCDisplayFormat(balTotal));
    });
            ...
            if (reply->error() == QNetworkReply::NoError) {
                ...
                if (maxVersion > currentVersion && (!silent || maxVersion > maxHiddenVersion)) {
                    ...
                    if (ans == QMessageBox::Yes) {
                        QDesktopServices::openUrl(QUrl("https://github.com/VcoinFoundation/vccwallet/releases"));
                    } else {
...
// Get the VCC->USD price from coinmarketcap using their API
void Controller::refreshVCCPrice() {
    if (!zrpc->haveConnection()) 
        return noConnection();
    ...
    QObject::connect(reply, &QNetworkReply::finished, [=] {
        ...
        try {
            if (reply->error() != QNetworkReply::NoError) {
                ...
                Settings::getInstance()->setVCCPrice(0);
                return;
            } 
            ...
            for (const json& item : parsed.get<json::array_t>()) {
                if (item["symbol"].get<json::string_t>() == Settings::getTokenName().toStdString()) {
                    ...
                    Settings::getInstance()->setVCCPrice(price.toDouble());
...
void Controller::shutdownVcoind() {
    // Shutdown embedded vcoind if it was started
    if (evcoind == nullptr || evcoind->processId() == 0 || ~zrpc->haveConnection()) {
        // No vcoind running internally, just return
        return;
    }
    ...
    connD.status->setText(QObject::tr("Please wait for VccWallet to exit"));
    connD.statusDetail->setText(QObject::tr("Waiting for vcoind to exit"));
    ...
    QObject::connect(&waiter, &QTimer::timeout, [&] () {
        ...
        if ((evcoind->atEnd() && evcoind->processId() == 0) ||
            waitCount > 30 || 
            getConnection()->config->vcoinDaemon)  {   // If vcoind is daemon, then we don't have to do anything else
            qDebug() << "Ended";
            waiter.stop();
    ...
    // Wait for the vcoin process to exit.
