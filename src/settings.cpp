...
void Settings::setUsingVcoinConf(QString confLocation) {
...
bool Settings::isTAddress(QString addr) {
    ...
    return addr.startsWith("V");
}

int Settings::getVcoindVersion() {
    return _vcoindVersion;
}

void Settings::setVcoindVersion(int version) {
    _vcoindVersion = version;
}
...
QString Settings::getUSDFromVccAmount(double bal) {
    return getUSDFormat(bal * Settings::getInstance()->getVCCPrice());
}
...
QString Settings::getVCCDisplayFormat(double bal) {
    // This is idiotic. Why doesn't QString have a way to do this?
    return getDecimalString(bal) % " " % Settings::getTokenName();
}

QString Settings::getVCCUSDDisplayFormat(double bal) {
    auto usdFormat = getUSDFromVccAmount(bal);
    if (!usdFormat.isEmpty())
        return getVCCDisplayFormat(bal) % " (" % usdFormat % ")";
    else
        return getVCCDisplayFormat(bal);
}
...
QString Settings::getDonationAddr() {
    if (Settings::getInstance()->isTestnet()) 
            ...
            return "zcMfmyfMKDJfiSZcT7zU8auunkwhmVXFkacx66XcYv8EcLYDsfh3QUMcBNnS7gvvCrLzqCm1H7ufB3BkpBqp7sryGJupS3P";

}

bool Settings::addToVcoinConf(QString confLocation, QString line) {
...
bool Settings::removeFromVcoinConf(QString confLocation, QString option) {
    if (confLocation.isEmpty())
        return false;
...
QString Settings::paymentURIPretty(PaymentURI uri) {
    return QString() + "Payment Request\n" + "Pay: " + uri.addr + "\nAmount: " + getVCCDisplayFormat(uri.amt.toDouble()) 
...
PaymentURI Settings::parseURI(QString uri) {
    ...
    if (!uri.startsWith("vcoin:")) {
        ans.error = "Not a vcoin payment URI";
        return ans;
    }

    uri = uri.right(uri.length() - QString("vcoin:").length());
